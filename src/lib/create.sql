...
ON scraper_health (support_status, validation_status, last_run_at DESC);

ALTER TABLE scraper_health ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "public read access" ON scraper_health;
CREATE POLICY "public read access"
ON scraper_health FOR SELECT
TO anon, authenticated
USING (true);
REVOKE ALL ON TABLE scraper_health FROM anon, authenticated;
GRANT SELECT ON TABLE scraper_health TO anon, authenticated;
GRANT ALL ON TABLE scraper_health TO service_role;
REVOKE ALL ON SEQUENCE scraper_health_id_seq FROM anon, authenticated;
GRANT USAGE, SELECT ON SEQUENCE scraper_health_id_seq TO service_role;
